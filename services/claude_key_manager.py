...
CREDENTIALS_FILENAME = "claude_credentials.json"
CREDENTIALS_FIELD = "anthropic_api_key"


class ClaudeKeyManager:
    """
    Shared M12 Anthropic (Claude) credential storage.

    Mirrors APIKeyManager exactly so both AI providers behave the
    same way from Settings and from any screen that needs a key.
    ...
    Desktop:
        ...
        ANTHROPIC_API_KEY is supported only as a fallback.
    """
    ...
    def get_api_key(cls):
        """
        Return the active Anthropic API key.

        Priority on every platform:
        1. Key explicitly saved in M12 Settings/private storage.
        2. ANTHROPIC_API_KEY environment variable as fallback.
        """
    ...
    def key_source(cls):
        ...
        if environment_key:
            return "ANTHROPIC_API_KEY environment"
    ...
    def delete_api_key(cls):
        ...
        try:
            ...
            raise RuntimeError(
                "Unable to remove the saved Claude API key: "
                f"{type(error).__name__}: {error}"
            ) from error
